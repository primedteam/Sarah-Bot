module.exports.run = async (Client, message, args, Con, Discord, Config) => {
    if(message.member.roles.cache.some(role => role.id === Config.roles.executives) || message.member.roles.cache.some(role => role.id === Config.roles.genmanager)){
        ...
        Con.query(`SELECT * FROM bannedUsers WHERE UserID = '${user.id}';`,async (error, result) => {
            ...
            if(result.length <= 0){
                Con.query(`INSERT INTO bannedUsers (UserID, CreatedAt) VALUES ('${user.id}', '${Math.floor(Date.now()/1000)}')`, async (error, result) => {
                ...
                return message.channel.send(new Discord.MessageEmbed().setColor(Config.colors.error).setDescription(`The user is already banned!`)).then((msg) => {msg.delete({timeout: 10000, reason: 'Deleted after 10s delay'})})
            }
        })
    } else {
...
module.exports.help = {
  name: "block",
  desc: "Blocks the user from using the bot.",
