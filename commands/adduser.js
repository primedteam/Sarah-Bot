module.exports.run = async (Client, message, args, Con, Discord, Config) => {
    Con.query(`SELECT * FROM Tickets WHERE ChannelID = '${message.channel.id}'`, async (error, result) => {
        ...
        channel.updateOverwrite(UserAdd, {SEND_MESSAGES: true, VIEW_CHANNEL: true}, `Added ${UserAdd} to a support ticket!`).then(async () => {
            return channel.send(new Discord.MessageEmbed().setColor(Config.colors.complete).setTitle(`Added user!`).setDescription(`I added ${UserAdd} to your support ticket!`).setTimestamp().setFooter('Your Welcome! ')).then(async (msg) => { msg.delete({timeout: 30000, reason: 'Removed Added User!'}); });
        }).catch(() => {})
    })
}
